...
export async function getShowSpecialEvent(): Promise<SpecialEvent | null> {
  return unstable_cache(
    ...
    ['show-special-event'],
    // Short TTL on top of the afterChange revalidation hooks: the banner is
    // admin-toggled and must never stay up long if a hook misfires.
    { tags: ['show-special-event', 'special-events'], revalidate: 60 * 60 },
  )()
}
